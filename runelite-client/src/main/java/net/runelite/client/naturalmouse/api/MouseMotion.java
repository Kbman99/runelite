...
public class MouseMotion {
   ...
   */
  public MouseMotion(Component source, MouseMotionNature nature, Random random, int xDest, int yDest) {
    this.source = source;
  ...
  public void move(MouseMotionObserver observer) throws InterruptedException {
    log.info("Starting move function");
    updateMouseInfo();
    ...
    while (mousePosition.x != xDest || mousePosition.y != yDest) {
      ...
      for (int i = 0; i < steps; i++) {
        ...
        systemCalls.setMousePosition(source,
            mousePosX,
            mousePosY
        );
      ...
      if (mousePosition.x != movement.destX || mousePosition.y != movement.destY) {
        ...
        systemCalls.setMousePosition(source, movement.destX, movement.destY);
  ...
  private void updateMouseInfo() {
    ...
    mousePosition = systemCalls.getMousePosition();
  }
}
